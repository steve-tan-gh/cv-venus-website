...
import { ArrowLeft, Save, Trash2, Calendar, Target, Percent, Gift } from "lucide-react"
...
import type { Category, Brand, Product, Discount } from "@/lib/supabase"
...
export default function EditDiscountPage() {
  ...
  const fetchData = async () => {
    try {
      const [discountRes, categoriesRes, brandsRes, productsRes] = await Promise.all([
        supabase.from("discounts").select("*").eq("id", discountId).single(),
      ...
      setFormData({
        ...
        applies_to_id: discountData.applies_to_id,
        free_quantity: discountData.free_quantity || 1,
        discount_percentage: discountData.discount_percentage || 10,
        is_active: discountData.is_active,
        start_date: discountData.start_date ? new Date(discountData.start_date).toISOString().slice(0, 16) : "",
        end_date: discountData.end_date ? new Date(discountData.end_date).toISOString().slice(0, 16) : "",
      })
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("discounts").update(discountData).eq("id", discountId)
      ...
      <div className="min-h-screen text-white">
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-16">
            <h1 className="text-2xl font-bold mb-4">Discount not found</h1>
            <Button onClick={() => router.push("/admin/discounts")}>Back to Discounts</Button>
          </div>
        </div>
      </div>
    )
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold mb-2">Edit Discount</h1>
              <p className="text-blue-200">Update discount settings</p>
            </div>
            <AlertDialog>
